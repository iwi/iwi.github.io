...
Plenty of the above is problematic when the goal is to communicate information in a format that's easy to digest. Specifically:
  ...
  - The siloing of sections into slides has the effect of making it particularly difficult to extend content. Updating a slide or adding information to it either requires a full re-design. The actual consequence is usually that the few empty space in the slice gets filled in with the extra info and any previous design is broken.
